...
func _ready():
	value = catstats.calico_cat_hunger
	tint_progress = Color(0.26, 0.77, 0.19, 1)
	get_node("Timer").wait_time = catstats.calico_cat_time_left
	get_node("Timer").start(get_node("Timer").wait_time)

func _process(delta):
	value = get_node("Timer").time_left
	catstats.calico_cat_time_left = get_node("Timer").time_left
	catstats.calico_cat_hunger = value
	if (value > 70):
		tint_progress = Color(0.26, 0.77, 0.19, 1)
